"""
Elo Margin Calibration Evaluation v0.5.0 for BTA Rugby Analytics.

Challenger model:
- EloMarginCalibration

Purpose:
Keep the existing Elo team-strength and win-probability logic unchanged, but
replace the fixed Elo-to-margin scale with a linear calibration learned only
from seasons before the evaluation season.

For each evaluation season:
    actual_home_margin = intercept + beta * adjusted_pre_match_elo_difference

Point-in-time protections:
- Matches are processed in stable chronological order: MatchDate, MatchID.
- Elo predictions are made before the target match updates model state.
- Margin calibration is fit only on completed matches from seasons before the
  evaluation season.
- Scheduled matches never update model state or evaluation metrics.
"""
...
def fit_linear_margin_calibration(training_matches: list[Match]) -> tuple[float, float, int]:
    """Fit margin = intercept + beta * adjusted Elo difference using prior seasons only."""
    ...
    for match in sort_matches(training_matches):
        ...
        home_rating = state.ratings[match.home_team_id]
        away_rating = state.ratings[match.away_team_id]
        adjusted_elo_diff = home_rating + HOME_ADVANTAGE - away_rating
        actual_margin = float(match.home_score - match.away_score)

        x_values.append(adjusted_elo_diff)
        y_values.append(actual_margin)

        update_state_after_match(state, match, update_season_margins=False)

    n = len(x_values)
    if n < 2:
        return 0.0, ELO_MARGIN_POINTS_SCALE, n
...
def make_prediction(
    ...
    margin_beta: float = ELO_MARGIN_POINTS_SCALE,
) -> Prediction:
    home_rating = state.ratings[match.home_team_id]
    away_rating = state.ratings[match.away_team_id]
    ...
    if model_name == "EloMarginCalibration":
        elo_diff = home_rating + HOME_ADVANTAGE - away_rating

        # Preserve incumbent Elo probability logic exactly.
        ...
        home_p, draw_p, away_p = normalise_probabilities(
            raw_home * (1.0 - draw_p),
            draw_p,
            (1.0 - raw_home) * (1.0 - draw_p),
        )

        # Challenger changes only the margin layer.
        predicted_margin = margin_intercept + margin_beta * elo_diff
    else:
...
def run_model_for_season(
    model_name: str,
    matches: list[Match],
    evaluation_season: int,
) -> tuple[list[Prediction], list[dict], list[dict], dict]:
    ...
    margin_intercept, margin_beta, calibration_n = fit_linear_margin_calibration(train)
    summary["margin_intercept"] = margin_intercept
    summary["margin_beta"] = margin_beta
    summary["margin_calibration_n"] = calibration_n
    ...
    for match in eval_matches:
        ...
        # Same-season results become available only after their prediction.
        # Calibration parameters remain fixed for the evaluation season because they
        # were estimated using prior seasons only.
...
def ensure_model_version(cursor, model_name: str, summary: dict) -> int:
    notes = (
        "Elo Margin Calibration Evaluation v0.5.0; walk-forward by season; "
        "Elo win probability logic unchanged from EloOnlyBaseline v0.2.0; "
        "margin fitted on prior seasons only as intercept + beta * adjusted Elo difference; "
        f"BASE_ELO={BASE_ELO}; HOME_ADVANTAGE={HOME_ADVANTAGE}; K_FACTOR={K_FACTOR}; "
        f"margin_intercept={summary.get('margin_intercept', 0.0):.6f}; "
        f"margin_beta={summary.get('margin_beta', ELO_MARGIN_POINTS_SCALE):.6f}; "
        f"margin_calibration_n={summary.get('margin_calibration_n', 0)}"
    )
    cursor.execute(
        """
    ...
    for model_name in models:
        for season in eval_seasons:
            ...
            print(
                ...
                f"intercept={summary.get('margin_intercept', 0.0):.3f} "
                f"beta={summary.get('margin_beta', ELO_MARGIN_POINTS_SCALE):.5f} "
                f"cal_n={summary.get('margin_calibration_n', 0)}"
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Elo Margin Calibration Evaluation v0.5.0")
    parser.add_argument("--model", default="EloMarginCalibration", help="Model name or 'all'")
