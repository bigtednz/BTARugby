"""
Damped Margin Elo Evaluation v0.3.0 for BTA Rugby Analytics.

Challenger model:
- DampedMarginElo

Purpose:
Test whether reducing the influence of large winning margins on Elo updates
improves probability calibration, winner accuracy and margin prediction.
...
def margin_multiplier(margin: float) -> float:
    return 1.0 + MARGIN_DAMPING_WEIGHT * math.log(abs(margin) + 1.0)
...
def make_prediction(model_name: str, match: Match, state: ModelState, summary: dict, evaluation_season: int) -> Prediction:
    ...
    elif model_name == "DampedMarginElo":
...
def ensure_model_version(cursor, model_name: str, summary: dict) -> int:
    notes = (
        "Damped Margin Elo Evaluation v0.3.0; walk-forward by season; "
        "predictions are made before same-match state updates; "
        f"BASE_ELO={BASE_ELO}; HOME_ADVANTAGE={HOME_ADVANTAGE}; "
        f"K_FACTOR={K_FACTOR}; MARGIN_DAMPING_WEIGHT={MARGIN_DAMPING_WEIGHT}; "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Damped Margin Elo Evaluation v0.3.0")
