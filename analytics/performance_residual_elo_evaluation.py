"""
Performance Residual Elo Evaluation v0.4.0 for BTA Rugby Analytics.

Challenger model:
- PerformanceResidualElo

Purpose:
Test whether recent opponent- and venue-adjusted performance residuals improve
Elo-based win probability and margin prediction.

Point-in-time protections:
- Matches are processed in stable chronological order: MatchDate, MatchID.
- Predictions are made before the target match updates model state.
- Residual features use only prior completed matches.
...
PERFORMANCE_RESIDUAL_WEIGHT = 0.50
...
class ModelState:
    ...
    performance_residuals: defaultdict
...
def initialise_state(training_matches: list[Match]) -> ModelState:
    state = ModelState(
        ...
        performance_residuals=defaultdict(lambda: deque(maxlen=5)),
...
def update_state_after_match(state: ModelState, match: Match, update_season_margins: bool = True) -> None:
    ...
    # Calculate performance against the pre-match Elo expectation.
    expected_margin = (home_rating + HOME_ADVANTAGE - away_rating) * ELO_MARGIN_POINTS_SCALE
    performance_residual = margin - expected_margin
    ...
    state.performance_residuals[match.home_team_id].append(performance_residual)
    state.performance_residuals[match.away_team_id].append(-performance_residual)
...
def make_prediction(model_name: str, match: Match, state: ModelState, summary: dict, evaluation_season: int) -> Prediction:
    ...
    if model_name == "PerformanceResidualElo":
        elo_diff = home_rating + HOME_ADVANTAGE - away_rating
        home_residual = average(state.performance_residuals[match.home_team_id]) or 0.0
        away_residual = average(state.performance_residuals[match.away_team_id]) or 0.0
        residual_edge = home_residual - away_residual

        predicted_margin = (
            elo_diff * ELO_MARGIN_POINTS_SCALE
            + residual_edge * PERFORMANCE_RESIDUAL_WEIGHT
        )
...
def ensure_model_version(cursor, model_name: str, summary: dict) -> int:
    notes = (
        "Performance Residual Elo Evaluation v0.4.0; walk-forward by season; "
        "predictions are made before same-match state updates; "
        "recent residual = actual margin minus Elo-expected margin; "
        f"BASE_ELO={BASE_ELO}; HOME_ADVANTAGE={HOME_ADVANTAGE}; "
        f"K_FACTOR={K_FACTOR}; ELO_MARGIN_POINTS_SCALE={ELO_MARGIN_POINTS_SCALE}; "
        f"PERFORMANCE_RESIDUAL_WEIGHT={PERFORMANCE_RESIDUAL_WEIGHT}"
    )
    cursor.execute(
        """
...
def run(args: argparse.Namespace) -> None:
    global PERFORMANCE_RESIDUAL_WEIGHT
    PERFORMANCE_RESIDUAL_WEIGHT = args.residual_weight
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Performance Residual Elo Evaluation v0.4.0")
    parser.add_argument("--model", default="PerformanceResidualElo", help="Model name or 'all'")
    parser.add_argument("--evaluation-season", default="all", help="Evaluation season or 'all'")
    parser.add_argument("--residual-weight", type=float, default=0.50, help="Weight applied to the home-minus-away recent performance residual edge")
